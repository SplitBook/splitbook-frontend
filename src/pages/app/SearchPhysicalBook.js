...
export default function SearchPhysicalBook(){
  ...
  if(bookstates.length===0){
    ...
    function submitChanges(){
      ...
      api.put('/physical-books/'+physicalBook.id,{state_id:state,description:description,location_id:location})
      setOpen(false)
    }
    ...
    return (
      ...
      <Grid container spacing={1}>
            ...
            <Grid item >
              <Button variant="outlined" color="primary" style={{ padding: 14}} disabled={(physicalBook!==null && physicalBook!=='')? false:true} onClick={() => setOpen(true)}>
                <Edit/>
              </Button> 
            </Grid>
            <Grid item >
              <Button variant="outlined" color="primary" style={{ padding: 14}} disabled={(physicalBook!==null && physicalBook!=='')? false:true} onClick={()=> generateOneQRCode(physicalBook.id)}>
      ...
      >
        <DialogTitle id="alert-dialog-title">Editar informações do livro ID:{physicalBook.id}</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            ...
            <div className="margTop">
              <p><i>Descrição:</i></p>
              <textarea cols={50} rows={15} defaultValue={physicalBook.description}  onChange={(e) => setDescription(e.target.value)}/>
            </div>
          </DialogContentText>
        </DialogContent>
        <DialogActions>
